  ...
  <div class="col-span-12">
    <div class="rounded-sm border border-stroke bg-white px-5 py-6 shadow-default dark:border-strokedark dark:bg-boxdark">
      <div class="flex flex-wrap items-center justify-between gap-3 mb-6">
        <div>
          <h2 class="text-xl font-semibold text-black dark:text-white">Ubah Kegiatan Program</h2>
          <p class="text-sm text-gray-500 mt-1">Perbarui tanggal, status, atau peserta.</p>
        </div>
        <a href="{{ route('program.activities.show', $activity->id) }}" class="inline-flex items-center rounded-lg border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 shadow-theme-xs hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-white/[0.03]">
          Lihat Detail
        </a>
      </div>

      <form action="{{ route('program.activities.update', $activity->id) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
          <div>
            ...
            <select id="program_id" name="program_id" required>
              ...
              @foreach($programs as $program)
                <option value="{{ $program->id }}" @selected(old('program_id', $activity->program_id) == $program->id)>{{ $program->name }}</option>
              @endforeach
            </select>
          </div>
          ...
          <div>
            ...
            <select id="division_id" name="division_id" required>
              ...
              @foreach($divisions as $division)
                <option value="{{ $division->id }}" @selected(old('division_id', $activity->division_id) == $division->id)>{{ $division->name }}</option>
              @endforeach
            </select>
          </div>
          ...
          <div>
            ...
            <select id="type" name="type" required>
              ...
              @foreach($types as $type)
                <option value="{{ $type }}" @selected(old('type', $activity->type) == $type)>{{ strtoupper(str_replace('_', ' ', $type)) }}</option>
              @endforeach
            </select>
          </div>
          ...
          <div>
            ...
            <select id="status" name="status">
              ...
              @foreach($statuses as $status)
                <option value="{{ $status }}" @selected(old('status', $activity->status) == $status)>{{ strtoupper(str_replace('_', ' ', $status)) }}</option>
              @endforeach
            </select>
          </div>
          ...
          <div>
            ...
            <input type="date" id="start_date" name="start_date" value="{{ old('start_date', optional($activity->start_date)->format('Y-m-d')) }}" required>
          </div>
          ...
          <div>
            ...
            <input type="date" id="end_date" name="end_date" value="{{ old('end_date', optional($activity->end_date)->format('Y-m-d')) }}" required>
          </div>
          ...
          <div>
            ...
            <input type="number" id="capacity" name="capacity" min="0" value="{{ old('capacity', $activity->capacity) }}">
          </div>
          ...
          <div>
            ...
            <input type="number" id="registered_count" name="registered_count" min="0" value="{{ old('registered_count', $activity->registered_count) }}">
          </div>
