  ...
  <div class="col-span-12">
    <div class="rounded-sm border border-stroke bg-white px-5 py-6 shadow-default dark:border-strokedark dark:bg-boxdark">
      <div class="flex flex-wrap items-center justify-between gap-3 mb-6">
        <h2 class="text-xl font-semibold text-black dark:text-white">Edit Program Kerja</h2>
      ...
      <form action="{{ route('program.programs.update', $program->id) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
          ...
          <div>
            ...
            >
              ...
              <option value="{{ $period->id }}" {{ old('period_id', $program->period_id) == $period->id ? 'selected' : '' }}>
                {{ $period->name }}
              </option>
              @endforeach
            </select>
          ...
          <div>
            ...
            >
              ...
              <option value="{{ $field->id }}" {{ old('field_id', $program->field_id) == $field->id ? 'selected' : '' }}>
                {{ $field->name }}
              </option>
              @endforeach
            </select>
          ...
          <div>
            ...
            >
              ...
              <option value="terprogram" {{ old('type', $program->type) == 'terprogram' ? 'selected' : '' }}>Terprogram</option>
              <option value="insidentil" {{ old('type', $program->type) == 'insidentil' ? 'selected' : '' }}>Insidentil</option>
            </select>
          </div>
          ...
          <div>
            ...
            <input 
              ...
              id="target" 
              name="target" 
              value="{{ old('target', $program->target) }}"
              min="0"
              class="w-full rounded border border-gray-300 bg-white px-4 py-2 text-black focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary dark:border-gray-600 dark:bg-boxdark dark:text-white"
            >
          </div>
          ...
          <div>
            ...
            >
              ...
              @foreach($statuses as $status)
                <option value="{{ $status }}" {{ old('status', $program->status) == $status ? 'selected' : '' }}>
