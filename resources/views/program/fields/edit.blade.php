      ...
      <form action="{{ route('program.fields.update', $field) }}" method="POST" class="space-y-6">
        ...
        <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
          <div>
            ...
            <select id="period_id" name="period_id" required>
              ...
              @foreach($periods as $period)
                <option value="{{ $period->id }}" @selected(old('period_id', $field->period_id) == $period->id)>{{ $period->name }}</option>
              @endforeach
            </select>
          </div>
          ...
          <div>
            ...
            <input type="text" id="code" name="code" value="{{ old('code', $field->code) }}" required>
          </div>
          ...
          <div>
            ...
            <select id="head_id" name="head_id">
              ...
              @foreach(\App\Models\Hr\Member::orderBy('name')->get() as $member)
                <option value="{{ $member->id }}" @selected(old('head_id', $field->head_id) == $member->id)>{{ $member->name }}</option>
              @endforeach
            </select>
          </div>
