  ...
  <div class="col-span-12">
    <div class="rounded-sm border border-stroke bg-white px-5 py-6 shadow-default dark:border-strokedark dark:bg-boxdark">
      <div class="flex flex-wrap items-center justify-between gap-3 mb-6">
        <h2 class="text-xl font-semibold text-black dark:text-white">Edit Anggota</h2>
        <a href="{{ route('hr.members.show', $member->id) }}" class="inline-flex items-center rounded-lg border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 shadow-theme-xs hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-white/[0.03]">
          Lihat Detail
        </a>
      </div>
      ...
      <form action="{{ route('hr.members.update', $member->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
          ...
          <div>
            ...
            <input 
              ...
              id="nia" 
              name="nia" 
              value="{{ old('nia', $member->profile->nia ?? '') }}"
              class="w-full rounded border border-gray-300 bg-white px-4 py-2 text-black focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary dark:border-gray-600 dark:bg-boxdark dark:text-white"
              required
            >
          </div>
          ...
          <div>
            ...
            >
              ...
              <option value="AM" {{ old('member_type', $member->member_type ?? 'AM') == 'AM' ? 'selected' : '' }}>Anggota Muda (AM)</option>
              <option value="AT" {{ old('member_type', $member->member_type) == 'AT' ? 'selected' : '' }}>Anggota Tetap (AT)</option>
              <option value="Alumni" {{ old('member_type', $member->member_type) == 'Alumni' ? 'selected' : '' }}>Alumni</option>
            </select>
          </div>
          ...
          <div>
            ...
            >
              ...
              <option value="terminated" {{ old('status', $member->status) == 'terminated' ? 'selected' : '' }}>Mengundurkan Diri</option>
            </select>
          </div>
          ...
          <div>
            ...
            @if($member->photo)
            <div class="mb-2">
              <img src="{{ asset('storage/members/' . $member->photo) }}" alt="{{ $member->name }}" class="h-20 w-20 rounded-full object-cover">
            </div>
            @endif
            <input 
              type="file" 
